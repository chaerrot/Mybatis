...
public class MybatisController {
	...
	public String list(Model model, HttpServletRequest req) {
		...
		/*
		 ...
		 동작방식] 컨트롤러에서 메서드 호출 -> interface의 추상메서드 호출
		 	-> namespace에 해당 interface를 namespave로 지정된 매퍼 선택
		 	-> 추상메서드와 동일한 이름의 id 속성을 가진 엘리먼트
		 */
		...
		int blockPage = 2; // 한 블럭 당 출력할 페이지 번호의 개수
		// 전체 페이지 수 계산
		int totalPage =
		...
		/*
		 ...
		 전다로딘 파라미터는 #{param1}과 같이 순서대로 사용한다.
		 */
	...
	public String write(Model model, HttpSession session,
		...
		if (session.getAttribute("siteUserInfo")==null) {
			/*
			 현재 상태는 글쓰기를 위해 버튼을 클릭했으므로, 만약 로그인 완료된다면
			 글쓰기 페이지로 이동하는 것이 좋다. 따라서 backUrl이라는 파라미터에
			 쓰기 페이지의 View경로를 붙여서 리다이렉트시킨다.
			 */
	...
	public ModelAndView loginAction (
		...
		if (vo == null) {
			...
			// 로그인에 성공한 경우 세션 영역에 MemberVO객체를 저장한다.
	...
	public String writeAction(Model model, HttpServletRequest req,
			HttpSession session) {
		/*
		 글쓰기 페이지에 오랫동안 머물러 세션이 끊어진느 경우가 있으므로
		...
		// insert문을 실행 시 입력에 성공한 행의 개수가 정수형으로 반환된다.
	...
	public String modify(Model model, HttpServletRequest req,
		...
		//수정페이지 진입 시에도 로그인 확인해야 함.
		if (session.getAttribute("siteUserInfo")==null) {
			return "redirect:login.do";
		}
		
		/*
		 파라미터를 전달하는 4번 째 방법으로 DTO(혹은 VO)객체에 파라미터를
		 저장한 후 Mapper로 전달한다.
		 */
	...
	public String modifyAction(HttpSession session, MyBoardDTO myBoardDTO) {
		...
		// 방명록 게시판은 상세보기 페이지가 별도로 없으므로 리스트로 돌아가면 된다.
		return "redirect:list.do";
	}
	// 삭제처리
